...
int setAllKeys(LedKeyboard &kbd, std::string arg2, bool commit = true)
{
	...
	if (!utils::parseColor(arg2, color))
		return 1;
	if (!kbd.open())
		return 1;
...
int setGroupKeys(LedKeyboard &kbd, std::string arg2, std::string arg3, bool commit = true)
{
	...
	LedKeyboard::Color color;
	if (!utils::parseKeyGroup(arg2, keyGroup))
		return 1;
	if (!utils::parseColor(arg3, color))
		return 1;
	if (!kbd.open())
		return 1;
	if (!kbd.setGroupKeys(keyGroup, color))
		return 1;
	if (commit)
		if (!kbd.commit())
			return 1;
...
{
	uint8_t value;
	if (!utils::parseUInt8(arg2, value))
		return 1;
	if (!kbd.open())
		return 1;
	if (!kbd.setMRKey(value))
		return 1;
	return 0;
}
...
int setRegion(LedKeyboard &kbd, std::string arg2, std::string arg3)
{
	...
	if (!utils::parseColor(arg3, color))
		return 1;
	if (!utils::parseUInt8(arg2, region))
		return 1;
	if (kbd.setRegion(region, color))
		return 0;
	return 1;
}
		  ...
		  const std::string &arg2, const std::string &arg3, const std::string &arg4, const std::string &arg5)
{
	...
	LedKeyboard::Color color;
	if (!utils::parseNativeEffect(arg2, effect))
		return 1;
	if (!utils::parseNativeEffectPart(arg3, effectPart))
		return 1;
	...
	case LedKeyboard::NativeEffect::color:
		if (!utils::parseColor(arg4, color))
			return 1;
		break;
	case LedKeyboard::NativeEffect::breathing:
		...
		if (arg5 == "")
			return 1;
		if (!utils::parsePeriod(arg5, period))
			return 1;
		break;
	...
	case LedKeyboard::NativeEffect::cwave:
		if (!utils::parsePeriod(arg4, period))
			return 1;
		break;
	}
	...
	if (!kbd.setNativeEffect(effect, effectPart, period, color, storage))
		return 1;
...
int setStartupMode(LedKeyboard &kbd, std::string arg2)
{
	...
	if (!utils::parseStartupMode(arg2, startupMode))
		return 1;
	if (!kbd.open())
		return 1;
	if (kbd.setStartupMode(startupMode))
		return 0;
	return 1;
}
...
int setOnBoardMode(LedKeyboard &kbd, std::string arg2)
{
	...
	if (!utils::parseOnBoardMode(arg2, onBoardMode))
		return 1;
	if (!kbd.open())
		return 1;
	if (kbd.setOnBoardMode(onBoardMode))
		return 0;
	return 1;
}
	...
	while (!stream.eof())
	{
		...
		if (line.size() > 0 && line.substr(0, 1) != "#")
		{
			...
			while (line.size() > 0)
			{
				...
				if (argValue.substr(0, 1) == "$")
					argValue = vars[argValue.substr(1)];
				args.push_back(argValue);
				if (line.substr(0, ind) == line)
					line.clear();
				else
			...
			{
				if (setAllKeys(kbd, args[1], false) == 1)
					retval = 1;
			}
			else if (args[0] == "g" && args.size() > 2)
			{
				if (setGroupKeys(kbd, args[1], args[2], false) == 1)
					retval = 1;
			}
			...
			{
				if (setRegion(kbd, args[1], args[2]) == 1)
					retval = 1;
			}
			else if (args[0] == "mr" && args.size() > 1)
			{
				if (setMRKey(kbd, args[1]) == 1)
					retval = 1;
			}
			else if (args[0] == "mn" && args.size() > 1)
			{
				if (setMNKey(kbd, args[1]) == 1)
					retval = 1;
			}
			else if (args[0] == "gkm" && args.size() > 1)
			{
				if (setGKeysMode(kbd, args[1]) == 1)
					retval = 1;
			}
			else if (args[0] == "sm" && args.size() > 1)
			{
				if (setStartupMode(kbd, args[1]) == 1)
					retval = 1;
			}
			else if (args[0] == "obm" && args.size() > 1)
			{
				if (setOnBoardMode(kbd, args[1]) == 1)
					retval = 1;
			}
			else if (args[0] == "fx" && args.size() > 4)
			{
				if (setFX(kbd, args[1], args[2], args[3], args[4]) == 1)
					retval = 1;
			}
			else if (args[0] == "fx" && args.size() > 3)
			{
				if (setFX(kbd, args[1], args[2], args[3]) == 1)
					retval = 1;
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	while (argIndex < argc)
	{
		...
		{
			if (!utils::parseUInt16(argv[argIndex + 1], vendorID))
				return 1;
			argIndex += 2;
			continue;
		}
		...
		{
			if (!utils::parseUInt16(argv[argIndex + 1], productID))
				return 1;
			argIndex += 2;
			continue;
		}
		else if (argc > (argIndex + 1) && arg == "-tuk")
		{
			...
			if (!utils::parseUInt8(argv[argIndex + 1], kbdProtocol))
				return 1;
			switch (kbdProtocol)
			{
			case 1:
		...
		else if (arg == "--list-keyboards")
			return listKeyboards(kbd);
		else if (arg == "--help-keys")
		{
			help::keys(argv[0]);
			return 0;
		}
		else if (arg == "--help-effects")
		{
			help::effects(argv[0]);
			return 0;
		}
		else if (arg == "--help-samples")
		{
			help::samples(argv[0]);
			return 0;
		}
		...
		if (arg == "-c")
			return commit(kbd);
		else if (arg == "--print-device")
		{
			printDeviceInfo(kbd.getCurrentDevice());
			return 0;
		}
		else if (argc > (argIndex + 1) && arg == "-a")
			return setAllKeys(kbd, argv[argIndex + 1]);
		else if (argc > (argIndex + 2) && arg == "-g")
			return setGroupKeys(kbd, argv[argIndex + 1], argv[argIndex + 2]);
		else if (argc > (argIndex + 2) && arg == "-k")
			return setKey(kbd, argv[argIndex + 1], argv[argIndex + 2]);
		else if (argc > (argIndex + 1) && arg == "-mr")
			return setMRKey(kbd, argv[argIndex + 1]);
		else if (argc > (argIndex + 1) && arg == "-mn")
			return setMNKey(kbd, argv[argIndex + 1]);
		else if (argc > (argIndex + 1) && arg == "-an")
			return setAllKeys(kbd, argv[argIndex + 1], false);
		else if (argc > (argIndex + 2) && arg == "-gn")
			return setGroupKeys(kbd, argv[argIndex + 1], argv[argIndex + 2], false);
		else if (argc > (argIndex + 2) && arg == "-kn")
			return setKey(kbd, argv[argIndex + 1], argv[argIndex + 2], false);
		else if (argc > (argIndex + 2) && arg == "-r")
			return setRegion(kbd, argv[argIndex + 1], argv[argIndex + 2]);
		else if (argc > (argIndex + 1) && arg == "-gkm")
			return setGKeysMode(kbd, argv[argIndex + 1]);
		else if (argc > (argIndex + 1) && arg == "-p")
			return loadProfile(kbd, argv[argIndex + 1]);
		else if (arg == "-pp")
			return pipeProfile(kbd);
		...
		else if (argc > (argIndex + 1) && arg == "--startup-mode")
			return setStartupMode(kbd, argv[argIndex + 1]);
		else if (argc > (argIndex + 1) && arg == "--on-board-mode")
			return setOnBoardMode(kbd, argv[argIndex + 1]);
		else
		{
			help::usage(argv[0]);
			return 1;
