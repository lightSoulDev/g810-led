		...
		for (int i = 0; i < (int)SupportedKeyboards.size(); i++)
		{
			if (desc.idVendor == SupportedKeyboards[i][0])
			{
				if (desc.idProduct == SupportedKeyboards[i][1])
				{
					...
					if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iSerialNumber, buf, 256) >= 1)
						deviceInfo.serialNumber = string((char *)buf);
					if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iManufacturer, buf, 256) >= 1)
						deviceInfo.manufacturer = string((char *)buf);
					if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iProduct, buf, 256) >= 1)
						deviceInfo.product = string((char *)buf);
...
bool LedKeyboard::open(uint16_t vendorID, uint16_t productID, string serial)
{
	...
	if (!serial.empty())
	{
		...
		if (mbstowcs(tempSerial, serial.c_str(), 256) < 1)
			return false;
	...
	while (dev)
	{
		for (int i = 0; i < (int)SupportedKeyboards.size(); i++)
		{
			...
			{
				if (!serial.empty() && dev->serial_number != NULL && wideSerial.compare(dev->serial_number) != 0)
					break; // Serial didn't match
	...
	if (wideSerial.empty())
		m_hidHandle = hid_open(currentDevice.vendorID, currentDevice.productID, NULL);
	else
		m_hidHandle = hid_open(currentDevice.vendorID, currentDevice.productID, wideSerial.c_str());
	...
	if (cnt >= 0)
	{
		for (ssize_t i = 0; i < cnt; i++)
		{
			...
			if (vendorID != 0x0 && desc.idVendor != vendorID)
				continue;
			else if (productID != 0x0 && desc.idProduct != productID)
				continue;
			else if (!serial.empty())
			{
				if (desc.iSerialNumber <= 0)
					continue; // Device does not populate serial number
				...
				if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iSerialNumber, buf, 256) >= 1 && serial.compare((char *)buf) == 0)
				{
					...
					for (int i = 0; i < (int)SupportedKeyboards.size(); i++)
					{
						if (desc.idVendor == SupportedKeyboards[i][0])
						{
							...
							{
								if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iManufacturer, buf, 256) >= 1)
									currentDevice.manufacturer = string((char *)buf);
								if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iProduct, buf, 256) >= 1)
									currentDevice.product = string((char *)buf);
				...
				if (desc.idVendor == SupportedKeyboards[i][0])
				{
					if (desc.idProduct == SupportedKeyboards[i][1])
					{
						...
						if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iProduct, buf, 256) >= 1)
							currentDevice.product = string((char *)buf);
						if (libusb_get_string_descriptor_ascii(m_hidHandle, desc.iSerialNumber, buf, 256) >= 1)
							currentDevice.serialNumber = string((char *)buf);
	...
	if (dev == NULL)
		m_hidHandle = libusb_open_device_with_vid_pid(m_ctx, currentDevice.vendorID, currentDevice.productID);
	else
		libusb_open(dev, &m_hidHandle);
...
bool LedKeyboard::close()
{
	...
	if (m_hidHandle == NULL)
		return true;
	if (libusb_release_interface(m_hidHandle, 1) != 0)
		return false;
	if (m_isKernellDetached == true)
	{
		libusb_attach_kernel_driver(m_hidHandle, 1);
...
bool LedKeyboard::setKeys(KeyValueArray keyValues)
{
	...
	case KeyboardModel::g815:
		for (uint8_t i = 0; i < keyValues.size(); i++)
		{
			...
			if (KeyByColors.count(colorkey) == 0)
				KeyByColors.insert(pair<uint32_t, vector<KeyValue>>(colorkey, {}));
	...
	default:
		...
		for (uint8_t i = 0; i < keyValues.size(); i++)
		{
			...
			case LedKeyboard::KeyAddressGroup::indicators:
				if (SortedKeys[1].size() <= 5)
					SortedKeys[1].push_back(keyValues[i]);
				break;
			case LedKeyboard::KeyAddressGroup::multimedia:
				...
				case LedKeyboard::KeyboardModel::g810:
				case LedKeyboard::KeyboardModel::gpro:
					if (SortedKeys[2].size() <= 5)
						SortedKeys[2].push_back(keyValues[i]);
					break;
				default:
					break;
				}
			...
			case LedKeyboard::KeyAddressGroup::gkeys:
				...
				{
				case LedKeyboard::KeyboardModel::g910:
					if (SortedKeys[3].size() <= 9)
						SortedKeys[3].push_back(keyValues[i]);
					break;
				default:
					break;
				}
...
bool LedKeyboard::setGroupKeys(KeyGroup keyGroup, LedKeyboard::Color color)
{
	...
	for (uint8_t i = 0; i < keyArray.size(); i++)
		keyValues.push_back({keyArray[i], color});
...
bool LedKeyboard::setAllKeys(LedKeyboard::Color color)
{
	...
	case KeyboardModel::g910:
	case KeyboardModel::gpro:
		for (uint8_t i = 0; i < keyGroupLogo.size(); i++)
			keyValues.push_back({keyGroupLogo[i], color});
		for (uint8_t i = 0; i < keyGroupIndicators.size(); i++)
			keyValues.push_back({keyGroupIndicators[i], color});
		for (uint8_t i = 0; i < keyGroupMultimedia.size(); i++)
			keyValues.push_back({keyGroupMultimedia[i], color});
		for (uint8_t i = 0; i < keyGroupGKeys.size(); i++)
			keyValues.push_back({keyGroupGKeys[i], color});
		for (uint8_t i = 0; i < keyGroupFKeys.size(); i++)
			keyValues.push_back({keyGroupFKeys[i], color});
		for (uint8_t i = 0; i < keyGroupFunctions.size(); i++)
			keyValues.push_back({keyGroupFunctions[i], color});
		for (uint8_t i = 0; i < keyGroupArrows.size(); i++)
			keyValues.push_back({keyGroupArrows[i], color});
		for (uint8_t i = 0; i < keyGroupNumeric.size(); i++)
			keyValues.push_back({keyGroupNumeric[i], color});
		for (uint8_t i = 0; i < keyGroupModifiers.size(); i++)
			keyValues.push_back({keyGroupModifiers[i], color});
		for (uint8_t i = 0; i < keyGroupKeys.size(); i++)
			keyValues.push_back({keyGroupKeys[i], color});
		return setKeys(keyValues);
	default:
		return false;
	}
								  ...
								  NativeEffectStorage storage)
{
	...
	if (part == LedKeyboard::NativeEffectPart::all)
	{
		...
		case NativeEffectGroup::color:
			if (!setGroupKeys(LedKeyboard::KeyGroup::indicators, color))
				return false;
			if (!commit())
				return false;
			break;
	...
	case KeyboardModel::g413:
		...
		if (part == NativeEffectPart::logo)
			return true; // Does not have logo component
		break;
	case KeyboardModel::g410:
	case KeyboardModel::g512:
	case KeyboardModel::g513:
...
bool LedKeyboard::sendDataInternal(byte_buffer_t &data)
{
	...
	{
#if defined(hidapi)
		if (!open(currentDevice.vendorID, currentDevice.productID, currentDevice.serialNumber))
			return false;
